...
public final class LanguageModelSession: @unchecked Sendable {
    ...
    public struct Response<Content>: Sendable where Content: Generable, Content: Sendable {
        ...
        /// Images generated by the model as part of this response.
        ///
        /// This extracts all ``Transcript/ImageSegment`` values from the
        /// response transcript entries that were produced by server-side
        /// image generation (e.g. the Responses API `image_generation` tool).
        public var generatedImages: [Transcript.ImageSegment] {
            transcriptEntries.flatMap { entry -> [Transcript.ImageSegment] in
                guard case .response(let resp) = entry else { return [] }
                return resp.segments.compactMap { seg in
